...
## Copied from https://github.com/thigley986/Lambda-AWS-Automated-Alarm-Creation/blob/master/createalarmsevent.py

# SNS Topic Definition for EC2, EBS
ec2_sns = '<ARN TO SEND THE ALERT TO>'
ebs_sns = '<ARN TO SEND THE ALERT TO>'
...
# Create Metric "Status Check Failed (Instance) for 20 Minutes"
    ...
    for v in vol_id:
        ...
        cw.put_metric_alarm(
        AlarmName="%s %s %s High Volume Activity Warning" % (name_tag, instanceid, v.id),
    ...
    # Create Metric "Volume Idle Time <= 30 sec (of 5 minutes) for 60 Minutes"
        cw.put_metric_alarm(
        AlarmName="%s %s %s High Volume Activity Critical" % (name_tag, instanceid, v.id),
